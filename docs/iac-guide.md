# IaC ガイド — Kaleido Scan AWS インフラ自動化

## 目次

1. [IaC ツール選定](#1-iac-ツール選定)
2. [Kaleido Scan の AWS アーキテクチャについて](#2-kaleido-scan-の-aws-アーキテクチャについて)
...
## 2. Kaleido Scan の AWS アーキテクチャについて
### Kaleido Scan の構成
Kaleido Scan は認証不要・サーバーレス構成なので以下のように最適化させる
...
| | メモリ | 512 MB |
| | タイムアウト | 30 秒 |
| | 環境変数 | `DB_HOST`、`DB_SECRET_ARN`、`AI_PROVIDER=bedrock`、`AWS_DEFAULT_REGION` |
...
| **Bedrock** | モデル | `us.anthropic.claude-sonnet-4-5-20250929-v1:0` |
...
infra/
├── bin/
│   └── kaleido-scan.ts        # エントリーポイント（スタック列挙）
└── lib/
...
# 3. IAM ロール作成（CDK でも可）
#    信頼ポリシー: repo:your-org/kaleido-scan:ref:refs/heads/main
...
export class DatabaseStack extends cdk.Stack {
  ...
  constructor(scope: cdk.App, id: string, props: Props) {
    ...
    const cluster = new rds.DatabaseCluster(this, 'KaleidoDb', {
  ...
  constructor(scope: cdk.App, id: string, props: Props) {
    ...
    const fn = new lambda.Function(this, 'ScanHandler', {
      ...
      environment: {
        ...
        AI_PROVIDER:   'bedrock',
        // AWS_DEFAULT_REGION はLambdaランタイムが自動注入するため設定不要（予約済み環境変数）
      },
    });
    ...
    // ※ Bedrock は米国リージョン（us-east-1）で Claude Sonnet が使えるか事前確認すること。
...
const network  = new NetworkStack(app,  'KaleidoNetwork',  { env });
const database = new DatabaseStack(app, 'KaleidoDatabase', { env, vpc: network.vpc, lambdaSG: network.lambdaSG });
new LambdaStack(app, 'KaleidoLambda', { env, vpc: network.vpc, lambdaSG: network.lambdaSG, dbEndpoint: database.dbEndpoint, dbSecretArn: database.dbSecretArn });
```
...
# 5. 出力された API URL を確認
# Outputs:
#   KaleidoLambda.ApiUrl = https://xxxxxxxx.execute-api.us-east-1.amazonaws.com
```

#### 動作確認

```bash
curl -X POST https://xxxxxxxx.execute-api.us-east-1.amazonaws.com/scan/ranking \
  -F "image=@test.jpg"
# → JSON レスポンスが返れば成功
```
  ...
  constructor(scope: cdk.App, id: string, props?: cdk.StackProps) {
    ...
    new iam.Role(this, 'GithubActionsRole', {
      ...
      assumedBy: new iam.WebIdentityPrincipal(provider.openIdConnectProviderArn, {
        StringLike: {
          // progate-hackathon-an/Kaleido-Scan の main ブランチのみ許可
...
VITE_API_URL=https://xxxxxxxx.execute-api.us-east-1.amazonaws.com
```
