...
まず静止画をAI画像認識API（ローカル: Gemini 2.0 Flash / 本番: AWS Bedrock）に送信し、商品名とオーラを表示すべき領域の座標を受け取る。その後、バックエンド処理のとして商品名をもとにDBからランキング情報を取得し、最終的に以下の形式でレスポンスを返す。
...
ローカルは Gemini 2.0 Flash、本番は AWS Bedrock（Claude Sonnet）を使用。インターフェースは環境変数で切り替える。
