...
| #   | タイトル                                                                      | 区分     | 優先度 | 工数 | フェーズ |
| --- | ----------------------------------------------------------------------------- | -------- | ------ | ---- | -------- |
| 0   | [バックエンドスタブ実装](#0-バックエンドスタブ実装)                             | Backend  | 高     | S    | Phase 1  |
| 1   | [DBマイグレーション・モデル実装](#1-dbマイグレーションモデル実装)             | Backend  | 高     | S    | Phase 1  |
| 2   | [DB接続・シードデータ実装](#2-db接続シードデータ実装)                         | Backend  | 高     | M    | Phase 1  |
| 3   | [スキャンエンドポイント実装](#3-スキャンエンドポイント実装)                   | Backend  | 高     | L    | Phase 1  |
| 4   | [商品詳細エンドポイント実装](#4-商品詳細エンドポイント実装)                   | Backend  | 高     | S    | Phase 1  |
| 5   | [型定義・APIクライアント実装](#5-型定義apiクライアント実装)                   | Frontend | 高     | S    | Phase 1  |
| 6   | [カメラスキャン画面実装](#6-カメラスキャン画面実装)                           | Frontend | 高     | M    | Phase 1  |
| 7   | [オーラ表示実装](#7-オーラ表示実装)                                           | Frontend | 高     | M    | Phase 1  |
| 8   | [商品詳細ボトムシート実装](#8-商品詳細ボトムシート実装)                       | Frontend | 高     | M    | Phase 1  |
| 9   | [エラーモーダル・ローディング演出実装](#9-エラーモーダルローディング演出実装) | Frontend | 中     | S    | Phase 1  |
| 10  | [PWA対応実装](#10-pwa対応実装)                                                | Frontend | 中     | S    | Phase 1  |
| 11  | [急上昇モードエンドポイント実装](#11-急上昇モードエンドポイント実装)          | Backend  | 中     | M    | Phase 2  |
| 12  | [急上昇モードUI実装](#12-急上昇モードui実装)                                  | Frontend | 中     | M    | Phase 2  |
| 13  | [掘り出し物モードエンドポイント実装](#13-掘り出し物モードエンドポイント実装)  | Backend  | 低     | M    | Phase 2  |
| 14  | [掘り出し物モードUI実装](#14-掘り出し物モードui実装)                          | Frontend | 低     | M    | Phase 2  |
| 15  | [X共有機能実装](#15-x共有機能実装)                                            | Frontend | 低     | M    | Phase 3  |
...
フロントエンド開発をバックエンド実装と並行して進められるよう、固定レスポンスを返すスタブAPIを先に実装する。Issue 5〜10のフロント実装はこのスタブを前提に進め、Issue 3・4完成後にスタブを本実装へ差し替える。
...
- [ ] `ErrorResponse` ヘルパーをIssue 3で作成したものに統一
...
> **UIイメージ**: Issue 12 と同様に、iOSカメラアプリの撮影モード変更（画面下部の横スクロール選択バー）のようなUIに「掘り出し物」を追加する形が理想。最終的に「通常 / 急上昇 / 掘り出し物」の3モードを1つの選択バーで切り替えられるようにする。
...
- [ ] `frontend/src/components/ProductBottomSheet.tsx` に `ShareButton` を追加（Issue 8で作成したボトムシートの下部に配置）
