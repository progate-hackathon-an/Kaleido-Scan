...
| #   | タイトル                                                                      | 区分     | 優先度 | 工数 | フェーズ |
| --- | ----------------------------------------------------------------------------- | -------- | ------ | ---- | -------- |
| [Issue 2](https://github.com/progate-hackathon-an/Kaleido-Scan/issues/2)   | [バックエンドスタブ実装](#2-バックエンドスタブ実装)                             | Backend  | 高     | S    | Phase 1  |
| [Issue 3](https://github.com/progate-hackathon-an/Kaleido-Scan/issues/3)   | [DBマイグレーション・モデル実装](#3-dbマイグレーションモデル実装)             | Backend  | 高     | S    | Phase 1  |
| [Issue 4](https://github.com/progate-hackathon-an/Kaleido-Scan/issues/4)   | [DB接続・シードデータ実装](#4-db接続シードデータ実装)                         | Backend  | 高     | M    | Phase 1  |
| [Issue 5](https://github.com/progate-hackathon-an/Kaleido-Scan/issues/5)   | [スキャンエンドポイント実装](#5-スキャンエンドポイント実装)                   | Backend  | 高     | L    | Phase 1  |
| [Issue 6](https://github.com/progate-hackathon-an/Kaleido-Scan/issues/6)   | [商品詳細エンドポイント実装](#6-商品詳細エンドポイント実装)                   | Backend  | 高     | S    | Phase 1  |
| [Issue 7](https://github.com/progate-hackathon-an/Kaleido-Scan/issues/7)   | [型定義・APIクライアント実装](#7-型定義apiクライアント実装)                   | Frontend | 高     | S    | Phase 1  |
| [Issue 8](https://github.com/progate-hackathon-an/Kaleido-Scan/issues/8)   | [カメラスキャン画面実装](#8-カメラスキャン画面実装)                           | Frontend | 高     | M    | Phase 1  |
| [Issue 9](https://github.com/progate-hackathon-an/Kaleido-Scan/issues/9)   | [オーラ表示実装](#9-オーラ表示実装)                                           | Frontend | 高     | M    | Phase 1  |
| [Issue 10](https://github.com/progate-hackathon-an/Kaleido-Scan/issues/10) | [商品詳細ボトムシート実装](#10-商品詳細ボトムシート実装)                       | Frontend | 高     | M    | Phase 1  |
| [Issue 11](https://github.com/progate-hackathon-an/Kaleido-Scan/issues/11) | [エラーモーダル・ローディング演出実装](#11-エラーモーダルローディング演出実装) | Frontend | 中     | S    | Phase 1  |
| [Issue 12](https://github.com/progate-hackathon-an/Kaleido-Scan/issues/12) | [PWA対応実装](#12-pwa対応実装)                                                | Frontend | 中     | S    | Phase 1  |
| [Issue 13](https://github.com/progate-hackathon-an/Kaleido-Scan/issues/13) | [急上昇モードエンドポイント実装](#13-急上昇モードエンドポイント実装)          | Backend  | 中     | M    | Phase 2  |
| [Issue 14](https://github.com/progate-hackathon-an/Kaleido-Scan/issues/14) | [急上昇モードUI実装](#14-急上昇モードui実装)                                  | Frontend | 中     | M    | Phase 2  |
| [Issue 15](https://github.com/progate-hackathon-an/Kaleido-Scan/issues/15) | [掘り出し物モードエンドポイント実装](#15-掘り出し物モードエンドポイント実装)  | Backend  | 低     | M    | Phase 2  |
| [Issue 16](https://github.com/progate-hackathon-an/Kaleido-Scan/issues/16) | [掘り出し物モードUI実装](#16-掘り出し物モードui実装)                          | Frontend | 低     | M    | Phase 2  |
| [Issue 17](https://github.com/progate-hackathon-an/Kaleido-Scan/issues/17) | [X共有機能実装](#17-x共有機能実装)                                            | Frontend | 低     | M    | Phase 3  |
...
- [x] `frontend/src/utils/auraRenderer.test.ts` に `TestGetAuraConfig_Level5` テスト作成（`aura_level=5` で金色・最大サイズの設定が返ること）
- [x] `frontend/src/utils/auraRenderer.test.ts` に `TestGetAuraConfig_Level1` テスト作成（`aura_level=1` でグレー・最小サイズの設定が返ること）
- [x] `frontend/src/components/AuraCanvas.test.tsx` に `TestAuraCanvas_Render` テスト作成（`<canvas>` 要素が描画されること）

**🟢 GREEN**

- [x] `frontend/src/utils/auraRenderer.ts` に `AuraConfig` 型定義（`color: string`, `radius: number`, `opacity: number`）
- [x] `frontend/src/utils/auraRenderer.ts` に `AURA_LEVEL_CONFIG: Record<number, AuraConfig>` 定数定義（5段階分: Lv5=金, Lv4=青, Lv3=緑, Lv2=紫, Lv1=グレー）
- [x] `frontend/src/utils/auraRenderer.ts` に `renderAura(ctx: CanvasRenderingContext2D, item: DetectedItem, canvasWidth: number, canvasHeight: number): void` 実装（`createRadialGradient` で円形ハローグロー描画）
- [x] `frontend/src/components/AuraCanvas.tsx` に `AuraCanvas` コンポーネント実装（`items: DetectedItem[]` を受け取り全商品にオーラ描画。各オーラにクリックイベントを設定して `onItemSelect(item)` を呼ぶ）
- [x] `frontend/src/hooks/useScan.ts` に `useScan(): { scan: (file: File) => Promise<void>, result: ScanResponse | null, isLoading: boolean, error: string | null }` 実装（`postScanRanking` をラップ）

**🔵 REFACTOR**

- [x] オーラの描画パラメータ（サイズ比率・グロー半径）を `AURA_LEVEL_CONFIG` に集約（`radius` がグロー半径比率、`opacity` が最大透明度を保持）
...
- [x] `frontend/src/components/ProductBottomSheet.test.tsx` に `TestProductBottomSheet_ShowOnOpen` テスト作成（`isOpen=true` で商品名・ランクバッジが表示されること）
- [x] `frontend/src/components/ProductBottomSheet.test.tsx` に `TestProductBottomSheet_HideOnClose` テスト作成（`isOpen=false` で非表示になること）

**🟢 GREEN**

- [x] `frontend/src/components/ProductBottomSheet.tsx` に `ProductBottomSheet` コンポーネント実装（props: `isOpen: boolean`, `item: DetectedItem | null`, `croppedImageUrl: string | null`, `onClose: () => void`）
  - 商品画像（`croppedImageUrl` を `<img>` で表示。トリミング処理は `utils/cropImage.ts` に切り出し、親が生成して渡す）
  - ランクバッジ（例: 🏅 1位）
  - 商品名・商品説明
  - ドラッグハンドル（下スワイプで `onClose` 呼び出し）
- [x] `frontend/src/utils/cropImage.ts` に `cropImage(imageFile: File, boundingBox: BoundingBox): Promise<string>` 実装

**🔵 REFACTOR**

- [x] スワイプ検出ロジックをカスタムフック `useSwipeDown(onSwipe)` に切り出し（`frontend/src/hooks/useSwipeDown.ts`）
...
- [x] `frontend/public/manifest.json` のバリデーションテスト（`name`, `short_name`, `start_url`, `display: "standalone"`, `icons` フィールドが存在すること）

**🟢 GREEN**

- [x] `frontend/public/manifest.json` 作成（`name: "Kaleido Scan"`, `short_name: "KaleidoScan"`, `display: "standalone"`, `start_url: "/"`, `theme_color`, `background_color`, `icons` を含む）
- [x] `frontend/index.html` に `<link rel="manifest">` タグ追加
- [x] `frontend/index.html` に iOS向け `<meta name="apple-mobile-web-app-capable">` タグ追加
- [x] `frontend/vite.config.ts` に `vite-plugin-pwa` 設定追加（Service Worker: キャッシュ対象は静的アセットのみ）

**🔵 REFACTOR**

- [x] PWAアイコン（192x192, 512x512）の生成・配置
...
- [x] `backend/services/trending_service_test.go` に `TestTrendingService_GetRanking_GrowthRate` テスト作成（直近週が前週より増加している商品が上位になること）
- [x] `backend/services/trending_service_test.go` に `TestTrendingService_GetRanking_NoPrevWeek` テスト作成（前週データなし時に `prev_quantity=0` で扱われること）
- [x] `backend/handlers/scan_handler_test.go` に `TestScanTrending_Success` テスト作成（期待: HTTP 200, `detected_items` に `growth_rate` フィールドが含まれること）

**🟢 GREEN**

- [x] `backend/services/trending_service.go` に `TrendingService` struct 定義（`ai AIService`, `db *sql.DB`）
- [x] `backend/services/trending_service.go` に `TrendingResult` struct 定義（`ScanResult` に加え `GrowthRate float64` フィールドを追加）
- [x] `backend/services/trending_service.go` に `GetTrendingRanking(ctx context.Context, imageData []byte) ([]TrendingResult, error)` 実装（AI呼び出し → 急上昇ランキングクエリ実行 → 検出商品に growth_rate を付与）
- [x] `backend/handlers/scan_handler.go` に `ScanTrending(c *gin.Context)` 実装
- [x] `backend/routes/routes.go` に `POST /scan/trending` ルート追加

**🔵 REFACTOR**

- [x] `ScanService` と `TrendingService` の共通AI呼び出しロジックを `services/base_scan_service.go` に切り出し
...
- [x] `frontend/src/components/CameraView.test.tsx` に `TestCameraView_RenderTabs` テスト作成（3つのモードタブが描画されること）※ `ModeToggle.test.tsx` の代替として CameraView に統合

**🟢 GREEN**

- [x] `frontend/src/types/scan.ts` に `ScanMode` 型追加（`'ranking' | 'hidden-gems' | 'trending'`）※ TrendingResponse は ranking と同構造のため ScanResponse を共用
- [x] `frontend/src/api/scanApi.ts` に `postScan(imageFile, mode)` 実装（3モード対応）
- [x] `frontend/src/hooks/useScan.ts` を `scan(file, mode)` に拡張（useTrending 相当を統合）
- [x] `frontend/src/components/CameraView.tsx` にモード切替タブ実装（ModeToggle.tsx 相当を CameraView に統合）
- [x] `frontend/src/utils/auraRenderer.ts` に急上昇モード対応（ranking と同カラーマップ使用。mode を renderAura に渡す仕組みを追加）

**🔵 REFACTOR**

- [x] `useScan(mode)` として Issue #16 の最終統合ゴールを直接実装（useRanking / useTrending を分割せず一本化）
...
- [x] `backend/services/hidden_gems_service_test.go` に `TestHiddenGemsService_GetRanking_ReverseOrder` テスト作成（売上最下位の商品の `aura_level` が最大になること）
- [x] `backend/handlers/scan_handler_test.go` に `TestScanHiddenGems_Success` テスト作成（期待: HTTP 200, `detected_items` が配列で返ること）

**🟢 GREEN**

- [x] `backend/services/hidden_gems_service.go` に `HiddenGemsService` struct 定義
- [x] `backend/services/hidden_gems_service.go` に `GetHiddenGemsRanking(ctx context.Context, imageData []byte) ([]ScanResult, error)` 実装（全期間累計の逆順で `aura_level` を付与: `aura_level = rank`）
- [x] `backend/handlers/scan_handler.go` に `ScanHiddenGems(c *gin.Context)` 実装
- [x] `backend/routes/routes.go` に `POST /scan/hidden-gems` ルート追加

**🔵 REFACTOR**

- [x] `aura_level` 計算ロジックを `services/aura.go` の `CalcAuraLevel(rank int, mode string) int` に集約
...
- [x] `frontend/src/hooks/useScan.test.ts` が `postScan(file, mode)` を検証（useHiddenGems 相当をカバー）

**🟢 GREEN**

- [x] `frontend/src/api/scanApi.ts` の `postScan(file, 'hidden-gems')` が `/scan/hidden-gems` を呼ぶ
- [x] `frontend/src/hooks/useScan.ts` が hidden-gems モードに対応（useHiddenGems 統合済み）
- [x] `frontend/src/utils/auraRenderer.ts` に `HIDDEN_GEMS_AURA_CONFIG` 定数定義（エメラルド/アメジスト/サファイア/ルビー/トパーズ）
- [x] `frontend/src/components/CameraView.tsx` の「掘り出しもの」タブで hidden-gems モードを選択可能

**🔵 REFACTOR**

- [x] `useScan(mode)` として統合完了（useRanking / useTrending / useHiddenGems を一本化）
...
- [x] `frontend/src/hooks/useShare.ts` に `useShare(): { share: (canvas, items, topItem) => Promise<void>, isSupported: boolean, error: string | null }` 実装（`navigator.share({ files: [imageFile], text: "KaleidoScanで〇〇を発見しました！ #KaleidoScan" })`）
